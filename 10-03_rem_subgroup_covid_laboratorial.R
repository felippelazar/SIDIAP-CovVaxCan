...
write.table(print.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_tx_group_matched_rem_3_doses_all_levels.csv'), sep = ';')

print.temp <- print(temp.table, showAllLevels = F, nonnormal = c('charlson_index', vars_covid_tests))
write.table(print.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_tx_group_matched_rem_3_doses_clean.csv'), sep = ';')

print.temp <- print(temp.table, showAllLevels = F, nonnormal = c('charlson_index', vars_covid_tests), smd = T)
write.table(print.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_tx_group_matched_rem_3_doses_smd.csv'), sep = ';')
...
write.table(g.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_tx_group_eligible_matched_rem_3_doses_smd.csv'), sep = ';')

ggsave(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_tx_group_matched_rem_3_doses_smd.pdf'),
...
write.table(print.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_outcomes_tx_group_matched_rem_3_doses_all_levels.csv'), sep = ';')

print.temp <- print(temp.table, showAllLevels = F, nonnormal = vars_outcomes_time)
write.table(print.temp, file = here('Results', 'dose_3', 'sub_group_covid_lab', 'desc_outcomes_tx_group_matched_rem_3_doses_clean.csv'), sep = ';')
...
ggsave(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_time_outcome_covid_matched_rem_3_doses.pdf'),
...
ggsave(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_time_outcome_hosp_death_matched_rem_3_doses.pdf'),
       dpi=600, height = 400*0.5, width=300*0.5, units = 'mm')
...
saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_covid.RDS'))
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_covid_rem_3_.pdf'))
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_covid_rem_3_confint.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
write.table(subgroup.temp.results,
            here('Results', 'dose_3', 'sub_group_covid_lab', 'subgroup_outcome_covid_three_periods.csv'), sep = ';', row.names = F)

#-- Outcome COVID-19 Hospitalization
fit <- survfit2(Surv(outcome_hosp_time, outcome_hosp_status == 2) ~ tx_group, 
                data = dfREMlong)

saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_hosp.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_rem_3_confint.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_rem_3_subset_0_30.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
write.table(subgroup.temp.results,
            here('Results', 'dose_3', 'sub_group_covid_lab', 'subgroup_outcome_hosp_three_periods.csv'), sep = ';', row.names = F)

# Outcome Severe COVID-19 Hospitalization
fit <- survfit2(Surv(outcome_hosp_severe_time, outcome_hosp_severe_status == 2) ~ tx_group, 
                data = dfREMlong)

saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_hosp_severe.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_severe_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_severe_rem_3_confint.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_death.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_death_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_death_rem_3_confint.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_hosp_death.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_death_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_death_rem_3_subset_0_30.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_hosp_death_rem_3_subset_0_14.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
write.table(subgroup.temp.results,
            here('Results', 'dose_3', 'sub_group_covid_lab', 'subgroup_outcome_hosp_death_three_periods.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'survfit2_outcome_noncovid_death.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'sub_group_covid_lab', 'graph_curve_noncovid_death_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
coxph(Surv(tstart, tstop, outcome == 1) ~ period, data = dfREM_death) %>% broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
      write.table(here('Results', 'dose_3', 'sub_group_covid_lab', 'outcome_noncovid_death_period_three.csv'), sep = ';', row.names = F)
...
saveRDS(cuminc_fit, here('Results', 'dose_3', 'sub_group_covid_lab', 'cuminc_outcome_death.RDS'))

crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfRem_death_cuminc, id = new_id, failcode = 'noncovid_death') %>%
      broom::tidy() %>% 
      write.table(here('Results', 'dose_3', 'sub_group_covid_lab', 'cuminc_outcome_noncovid_death_three_periods.csv'), sep = ';', row.names = F)

crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfRem_death_cuminc, id = new_id, failcode = 'covid_death') %>%
      broom::tidy() %>% 
      write.table(here('Results', 'dose_3', 'sub_group_covid_lab', 'cuminc_outcome_covid_death_three_periods.csv'), sep = ';', row.names = F)
