...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_covid.RDS'))
...
coxph(Surv(tstart, tstop, outcome == 2) ~ period + strata(subject_id_pair), 
      data = dfREM_covid) %>% broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   
  broom.helpers::tidy_add_reference_rows() %>%  broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_covid_period_three_stratified.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_hosp.RDS'))
...
coxph(Surv(tstart, tstop, outcome == 2) ~ period + strata(subject_id_pair), 
      data = dfREM_hosp) %>%   broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   
  broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_hosp_period_three_stratified.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_hosp_severe.RDS'))
...
coxph(Surv(tstart, tstop, outcome == 2) ~ period + strata(subject_id_pair), 
      data = dfREM_hosp_severe) %>% 
  broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_hosp_severe_period_three_stratified.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_death.RDS'))
...
coxph(Surv(tstart, tstop, outcome == 2) ~ period + strata(subject_id_pair), data = dfREM_death) %>% 
  broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_death_period_three_stratified.csv'), sep = ';', row.names = F)
...
write.table(subgroup.temp.results,
            here('Results', 'dose_3', 'rem_main_analysis', 'subgroup_outcome_death_three_periods.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_hosp_death.RDS'))
...
coxph(Surv(tstart, tstop, outcome == 2) ~ period + strata(subject_id_pair), 
      data = dfREM_hosp_death) %>% broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>%   
  broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_hosp_death_period_three_stratified.csv'), sep = ';', row.names = F)
...
saveRDS(fit, here('Results', 'dose_3', 'rem_main_analysis', 'survfit2_outcome_noncovid_death.RDS'))

temp.cumhaz <- ggsurvplot(fit, data = dfREMlong, fun = 'cumhaz', xlim = c(0, 180),
                          legend.labs = c("Control", "Vaccinated"),   break.x.by = 30, ggtheme = theme_bw(), 
                          palette = c("#E7B800","#2E9FDF"), risk.table = T)

pdf(here('Results', 'dose_3', 'rem_main_analysis', 'graph_curve_noncovid_death_rem_12_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
coxph(Surv(tstart, tstop, outcome == 1) ~ period, data = dfREM_death) %>% 
  broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>% 
  broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_noncovid_death_period_all.csv'), sep = ';', row.names = F)
...
coxph(Surv(tstart, tstop, outcome == 1) ~ period, data = dfREM_death) %>% 
  broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>% 
  broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_noncovid_death_period_three.csv'), sep = ';', row.names = F)

coxph(Surv(tstart, tstop, outcome == 1) ~ period + strata(subject_id_pair), data = dfREM_death) %>% 
  broom.helpers::tidy_and_attach(exponentiate=T, conf.int=T) %>% 
  broom.helpers::tidy_add_reference_rows() %>% broom.helpers::tidy_add_n() %>%
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'outcome_noncovid_death_period_three_stratified.csv'), sep = ';', row.names = F)
...
write.table(subgroup.temp.results,
            here('Results', 'dose_3', 'rem_main_analysis', 'subgroup_outcome_noncovid_death_three_periods.csv'), sep = ';', row.names = F)
...
crr(Surv(outcome_hosp_time, outcome_hosp_status) ~ period, data = dfREM_hosp_cuminc, id = new_id, failcode = 'noncovid_death') %>%
  broom::tidy() %>% 
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_hosp_three_periods_failcode_noncovid_death.csv'), sep = ';', row.names = F)

crr(Surv(outcome_hosp_time, outcome_hosp_status) ~ period, data = dfREM_hosp_cuminc, id = new_id, failcode = 'covid_hosp') %>%
  broom::tidy() %>% 
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_hosp_three_periods_failcode_covid_hosp.csv'), sep = ';', row.names = F)
...
saveRDS(cuminc_fit, here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_death.RDS'))

crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfREM_death_cuminc, id = new_id, failcode = 'noncovid_death') %>%
  broom::tidy() %>% 
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_death_three_periods_failcode_noncovid_death.csv'), sep = ';', row.names = F)

crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfREM_death_cuminc, id = new_id, failcode = 'covid_death') %>%
  broom::tidy() %>% 
  write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_death_three_periods_failcode_covid_death.csv'), sep = ';', row.names = F)
