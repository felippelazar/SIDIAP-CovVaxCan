...
pdf(here('Results', 'dose_3', 'rem_main_analysis', 'graph_curve_noncovid_death_rem_3_.pdf'))
print(temp.cumhaz, newpage = FALSE)
dev.off()
...
crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfRem_death_cuminc, id = new_id, failcode = 'noncovid_death') %>%
      broom::tidy() %>% 
      write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_noncovid_death_three_periods.csv'), sep = ';', row.names = F)

crr(Surv(outcome_death_time, outcome_death_status) ~ period, data = dfRem_death_cuminc, id = new_id, failcode = 'covid_death') %>%
      broom::tidy() %>% 
      write.table(here('Results', 'dose_3', 'rem_main_analysis', 'cuminc_outcome_covid_death_three_periods.csv'), sep = ';', row.names = F)
